...
type CloneNodeFunction<T extends HTMLElement = HTMLElement> = (
  dom: T,
  waitQueue: Array<Promise<void>>
) => T;

const cloneCanvasElement: CloneNodeFunction<HTMLCanvasElement> = (canvas) => {
...
const cloneImageElement: CloneNodeFunction<HTMLImageElement> = (
  image,
  waitQueue
) => {
...
const cloneVideoElement: CloneNodeFunction<HTMLVideoElement> = (
  video,
  waitQueue
) => {
...
const cloneInputElement: CloneNodeFunction<HTMLInputElement> = (input) => {
...
const cloneSelectElement: CloneNodeFunction<HTMLSelectElement> = (select) => {
  const clone = select.cloneNode() as HTMLSelectElement;
  clone.value = select.value;
  return clone;
};

const cloneOptionElement: CloneNodeFunction<HTMLOptionElement> = (option) => {
...
function cloneNode<T extends Node>(
  ...
  const cloneFunc = CLONE_FUNCTIONS.has(tagName)
    ? CLONE_FUNCTIONS.get(tagName)!
    : defaultCloneNode;
...
export async function deepCloneNode(
  dom: Node
): Promise<{ node: Node; result: PromiseSettledResult<void>[] }> {
  ...
  else if (CustomElementSupport.isCustomElement(dom))
    root = CustomElementSupport.cloneNode(dom);
  ...
  while (queue.length) {
    ...
    const parent = ShadowDomSupport.isShadowDom(mirror)
      ? mirror.shadowRoot!
      : mirror;
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      let clone: Node;
      if (ShadowDomSupport.isShadowDom(child))
        clone = ShadowDomSupport.cloneNode(child);
